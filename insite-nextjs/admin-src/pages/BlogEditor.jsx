...
export default function BlogEditor() {
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      const p = getPost(id)
      if (p) setPost(p)
      else router.push('/admin/dashboard/blog')
    }
  }, [id, isNew, router])
  ...
  const doSave = async (redirect = true) => {
    ...
    try {
      ...
      if (redirect && isNew) router.replace('/admin/dashboard/blog')
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, flexWrap: 'wrap' }}>
        <Link href="/admin/dashboard/blog" style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#8898b4', textDecoration: 'none', fontSize: 14, transition: 'color 0.15s', flexShrink: 0 }}
