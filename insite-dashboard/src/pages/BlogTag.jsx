        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {(post.tags || []).slice(0, 4).map((tag, i) => (
            <Link
              ...
              className="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-600 rounded-full text-xs hover:bg-insite-blue hover:text-white transition-colors"
            >
...
const BlogTag = () => {
  ...
  const displayTag =
    tag?.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()) || 'Tag';
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/blog/posts?tag=${encodeURIComponent(tag)}&status=published&limit=50`)
    ...
    return () => { cancelled = true; };
  }, [tag, currentLanguage]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="container-custom">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center space-x-2 text-white/90">
              ...
              <span>/</span>
              <span className="text-insite-cyan">{displayTag}</span>
            </div>
            {!loading && !error && (
