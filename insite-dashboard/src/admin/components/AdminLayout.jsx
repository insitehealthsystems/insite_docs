...
export default function AdminLayout() {
  ...
  useEffect(() => {
    if (!isAuthenticated()) { navigate('/admin/dashboard/login', { replace: true }) }
  }, [navigate])
  ...
  useEffect(() => { if (mobile) setSideOpen(false) }, [location, mobile])

  const handleLogout = () => { logout(); navigate('/admin/dashboard/login', { replace: true }) }
...
function Breadcrumb() {
  const location = useLocation()
  const parts = location.pathname.replace('/admin/dashboard', '').split('/').filter(Boolean)
